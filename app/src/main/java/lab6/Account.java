...
public class Account {
	...
	// TODO: Task3 
	// replace the null with a lambda expression
	public static AddMaker maker = N -> {return ((Account account) -> {account.balance += N;});};
	...
	public static int getMaxAccountID(List<Account> accounts) {
		// TODO: Task4 
		// replace the null with a lambda expression
		Account maxOne = accounts.stream().reduce(new Account(0, -100), (acc, n) -> {if(acc.balance < n.balance) {acc.balance = n.balance; acc.id = n.id;} return acc;});
